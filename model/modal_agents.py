...
def test():
    ...
    save_name = 'disent_diff_cond_128_vae'
    # save_name_ae = 'checkpoints/vae_cond_128_beta01_dis_same'
    save_name_ae = 'checkpoints/vae_cond_128_beta001_dis_same_100_frac'
    save_dir = f'D:/ModalAgent/evaluation/cond_gen_results_{MODEL_CLINET_VERSION}_union'
    os.makedirs(save_dir, exist_ok=True)
    latent_dim=128
    logic_mode = 'union'
    sample_max_num_nodes = 20
    ...
    print(dataset)
    split_idx = dataset.get_idx_split(len(dataset), train_size=8000, valid_size=1, seed=42)
    ...
    # test collaborate_between_agents_13
    ModalAgent = MetamatGenAgents(root=root, ckpt_dir=save_name_ae, device=device, backbone='vae',client_model=MODEL_CLINET_VERSION, evaluation_threshold=0.8, max_evaluate_num=10)
    ...
    for i in range(0,121):
        if conditional_generation:
            ...
            condition = test_batch.y
            young = test_batch.y[:, :3]
            shear = test_batch.y[:, 3:6]
            poisson = test_batch.y[:, 6:]
            input_text = init_prompt + f" Properties: Young's modulus: {young.tolist()[0]}, Shear modulus: {shear.tolist()[0]}, Poisson ratio: {poisson.tolist()[0]}."
        else:
            condition = None
        for j in range(5):
            print(f'====================> Generate index:{i}-{j}')
            z, coords, edge_index, batch, lengths, angles, num_atoms, evaluation_results_A1 = ModalAgent.collaborate_between_agents_13(input_text, visualize_results=False)

            # visualizeLattice(coords.cpu().numpy(), edge_index.cpu().numpy()) 
            
            # test collaborate_between_agents_12
            scaffold = z, coords, edge_index, batch, lengths, angles, num_atoms
            z, coords, edge_index, lengths_pred, angles_pred, evaluation_result, y_pred, y_cond, improved_prompt, final_output_prompt = \
                ModalAgent.collaborate_between_agents_23(select_init_data, scaffold, init_prompt + f" Properties: Young's modulus: {young.tolist()[0]}, Shear modulus: {shear.tolist()[0]}, Poisson ratio: {poisson.tolist()[0]}.",\
                                                          logic_mode=logic_mode, num_steps_lattice=100, \
                                                         num_steps_geo=300, optimize_network_params=False, fusion_thresh=0.5, mix_lam=0.1, condition=condition)
            # visualizeLattice(coords.cpu().numpy(), edge_index.cpu().numpy())
            print('lengths_pred', lengths_pred)
            print('angles_pred', angles_pred)
            print('y_pred', y_pred)
            ...
            print('final_output_prompt', final_output_prompt)
            input_text = improved_prompt
            if evaluation_result >= ModalAgent.evaluation_threshold:
                print(f"{j}th iter, Evaluation result: {evaluation_result} is greater than the threshold, stop the generation, and save the result.")
                break
        lattice_name = os.path.join(save_dir, f'{logic_mode}_{i}_{evaluation_result}.npz')
        np.savez(lattice_name,
    ...
    # batch_data = next(iter(DataLoader(dataset[58:59], batch_size=1, shuffle=False)))
    # visualizeLattice(batch_data.cart_coords.cpu().numpy(), batch_data.edge_index.cpu().numpy())


    # final_node_num, lengths_pred, angles_pred, cooreds_all, edge_index_all = ModalAgent.collaborate_between_agents_12(batch_data, scaffold, logic_mode='neg', num_steps_lattice=100, num_steps_geo=100, optimize_network_params=False, thresh=0.5)
