...
class GeomDecoder(nn.Module):
    ...
    def decode_semantic(self, semantic_emb, batch, return_emb=False, repeated_cond=None):
        ...
        # semantic_emb = self.shared_decoder(semantic_emb, batch, repeated_cond) 
    ...
    def forward(self, latent, disentangled_zs, num_nodes, batch, node_mask=None, sample_mode=False, condition=None):
        if self.is_condition:
            if condition is None:
                # condition = torch.randn((latent.shape[0], self.condition_dim), device=latent.device)
...
class GeomVAE(nn.Module):
    ...
    def compute_loss(self, batch_data, device='cuda', beta=1.0, beta_geo=1.0):
        z, coords, edge_index, batch = batch_data.node_type.to(device), batch_data.frac_coords.to(device), batch_data.edge_index.to(device), batch_data.batch.to(device)
        ...
        if self.is_variational:
            (node_num_logit, lengths_pred, angles_pred, coords_pred, edge_logits, edge_index_list, y_pred, node_mask), (mu, logvar, mu_geo, logvar_geo) = self(
                z, coords, edge_index, batch, lengths_normed, angles_normed, num_atoms, condition=condition
            )
        else:
            node_num_logit, lengths_pred, angles_pred, coords_pred, edge_logits, edge_index_list, y_pred, node_mask = self(
                z, coords, edge_index, batch, lengths_normed, angles_normed, num_atoms, condition=condition
            )
        ...
        recon_loss = loss_l + loss_a + 1000 * loss_coords + 1000 * loss_edge + loss_node_num
    ...
    def sample(self, num_samples=1, z_latent=None, encoded_coords=None, batch=None, device='cuda', condition=None, random_scale_geo=0.1, random_scale_latent=1.0, random_scale_cond=1e-3):
        ...
        if encoded_coords is not None:
            ...
            z0_e =  torch.randn((num_samples * node_num_pred.sum(), self.latent_dim), device=device)
            z0_p = torch.randn((num_samples * node_num_pred.sum(), self.latent_dim), device=device)
            z0_s = torch.randn((num_samples * node_num_pred.sum(), self.latent_dim), device=device)
